    ...
    return target.fromRest = function(config) {
      ...
      queryAndWriteInputBuffer = (function(_this) {
        return function(nextPage) {
          return config.query.apply(_this, [nextPage]).then(function(result, response) {
            return _this.from().writeArrayAsync(config.resultMapping(result, response)).done(function() {
